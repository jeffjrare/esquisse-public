...
**Conséquences:** The roadmap stops being the only place order exists and keeps what a flat list cannot hold — the groupings, the `needs:`/`unblocks:` edges and the `why now` prose. Its admission test still governs which items earn a written *entry*; it no longer governs which items have a *position*. `derive` resolves a divergent `Rank` as a projection rather than asking per row, and `esq validate` refuses a duplicate rank or an open row with no priority.
