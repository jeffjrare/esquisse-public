...
| D-one-skeleton-grep-two-carriers | 2026-08-17 | arch | build | `/esq:build` and `/esq:autopilot` derive the plan file's structure from one hash-enforced `shared:log-skeleton` grep, not from two independent regexes | Active — build's carriage amended 2026-09-15 by D-build-asks-the-cli-for-its-plan-context; build no longer carries the block since 2026-09-25 |
