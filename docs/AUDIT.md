...
- **Plan completion versus item acceptance** — exercise roadmap refresh (also reused by derive/edit) and sweep with a complete plan whose Planned item has an explicit unmet post-plan condition. The entry stays in its horizon, a dependent `needs:` stays blocked, sweep leaves the row open and no closure question or repeated build is offered. Repeat with missing evidence, then evidence of the whole outcome: only the latter earns reconciliation, and roadmap waits for the recorded disposition. All closed rows with complete plans/Done epics may ship; an active epic, missing reference or phase-less plan cannot. Check that relevant detail is read before either command concludes from a plan or commit, without scanning the whole backlog tail or buying optional research. This is a semantic reading pass, not a check for sentences in skill files (B-129).
