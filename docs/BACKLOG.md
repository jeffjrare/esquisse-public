...
| B-172 | 2026-09-20 | ✨ improvement | lo? | 150 | Assign an initial rank when adding a backlog row; keep the sequence complete without a separate rerank. | build: every-open-item-is-ranked Phase 3 |  |  | Open |
...
| B-179 | 2026-09-21 | debt | hi | 100 | Provide a bounded safe correction/disposition for a completed plan when corrective depth is exhausted. | observed: /esq:converge on observation-path-before-manual-fixes-fixes, 2026-09-21 | esq-decides-implementation-detail |  | Done |
...
**What:** Keep the existing backlog Rank coherent throughout capture, closure, reranking and reopening. `addRow` currently writes an empty Rank; `setStatus` retains a closed row's rank, and `rankOrder` can reuse it for active work, which `validate` then reports as a duplicate. Fix the lifecycle through the existing CLI writers.
...
**Fresh-session entry:** From this repository, load the current source plugin with `claude --plugin-dir ./plugin`, then invoke `/esq:work B-172`. Everything needed to understand this mandate is above; this chat is not a dependency. The item remains Open until implementation and verification deliver the whole acceptance.
