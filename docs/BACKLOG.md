...
| B-050 | 2026-08-19 | ☑️ todo | med? |  | Document headless verification permissions for a concrete bounded case, without a blanket bypass requirement. | build: relay-worker-model-separation Phase 4 · Done by b050-headless-permissions | aug-18-improv |  | Done |
...
**Resolution:** 2026-09-25: Delivered the bounded read-only recipe in README Operations, reusing the authorized B-182 success (130.49 s, 1.0563188 USD reported, 482 unchanged files). Host network authorization is distinct from model tools; isolated context/authentication, 3 USD / 180 s bounds and cleanup are explicit, without blanket bypass. No new model trial; restricted-network timeout cost remains unknown. Verification and limits: docs/preparation/2026-09-25-b050-headless.md. B-181/B-182 unchanged.
...
**Structural separation (2026-09-25):** Plan now prepares choices and verification before loading its file/ledger/branch contract; UI and authority guidance stay in preparation. The retained B-181 proposal is reused, never rerun. Identical archived Tamialog inputs and prompt produce no candidate: one zero-cost authentication startup failure, then ten unknown API retries and a 180-second timeout, with no final cost reported (configured cap $3). No output-quality, length or savings claim; B-182 remains Open. One audit has five initial passes, then only its two failed Node controls pass outside sandbox on unchanged sources. Exact change, comparison limits and evidence: [B-182 separation](preparation/2026-09-25-b182-tamialog.md).
