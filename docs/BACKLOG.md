...
| ---- | ------ | ------ | ----- | --- | --------- | -------- | ------ | --------- | -------- |
| B-001 | 2026-07-08 | ✨ improvement | lo? | 5800 | Backlog publish Phase 2 (partner comment round-trip) — parked until a cell-capable Google Sheets connector exists | plan: backlog-publish-google-sheet Phase 2 |  |  | Open |
...
| B-003 | 2026-08-09 | 🐛 bug | med? | 3100 | Removing a worktree with an unmerged branch frees its ID block for reuse, so a later worktree can re-mint that branch's B-NNN | build: prevent-ledger-id-collisions Phase 3 |  |  | Open |
| B-004 | 2026-08-09 | 🐛 bug | med? | 2800 | The citation-key invariant is replicated in 12 places but audit.sh check 10 guards only the 5 inside the id-allocation markers | build: prevent-ledger-id-collisions Phase 4 |  |  | Open |
| B-005 | 2026-08-10 | ⚠️ debt | med? | 3200 | Merge step 4's clean-auto-merge branch has no fixture — every existing one lands in step 3 or the abort path | fix: prevent-ledger-id-collisions-fixes |  |  | Open |
| B-006 | 2026-08-13 | 💡 idea | hi | 5700 | Port esquisse to Codex as a second install target, via a capability contract plus an install-time adapter layer | manual · Planned by harness-port-adapter-layer |  |  | Planned |
...
| B-010 | 2026-08-13 | 🐛 bug | med? | 2900 | The announce-open boilerplate is duplicated in 16 command files and, unlike the other shared blocks, is not hash-compared | build: long-commands-announce-their-bound Phase 2 |  |  | Open |
...
| B-012 | 2026-08-15 | ✨ improvement | lo? | 5500 | Trim commands/esq to the plan's 70,500 target — gap remeasured 2026-08-16 at 5,162 words, drifting ~2.5k/day unheld | manual · Planned by retire-legacy-rollback-path-fixes · Done by retire-legacy-rollback-path-fixes |  |  | Done |
| B-013 | 2026-08-15 | ⚠️ debt | med? | 4400 | check.md's six verdict sentences predate the three-zone headline and need a placement decision before any trim | check: command-set-cost-trim |  |  | Open |
| B-014 | 2026-08-15 | ⚠️ debt | med? | 4500 | ui.md's brief template shares its shape with /esq:grill's output contract — parity must be verified before trimming | check: command-set-cost-trim |  |  | Open |
...
| B-021 | 2026-08-17 | ☑️ todo | med? | 4600 | A phase editing commands/esq/ must verify with install.sh before audit.sh — check 10 reds by construction otherwise and the phase agent reads it as a failure | build: build-reconciles-before-executing Phase 1 · Planned by retire-legacy-rollback-path-fixes · Done by retire-legacy-rollback-path-fixes |  |  | Done |
| B-022 | 2026-08-18 | ⚠️ debt | lo | 2700 | The "No mandate, no run." guard is duplicated verbatim in 6 skills ×2 trees — register it in check-sharedblocks.sh so copies cannot drift | manual (D-orchestrated-skills-guard-in-body) |  |  | Open |
...
| B-024 | 2026-08-18 | 🐛 bug | med? | 4700 | spec offers "Je ne sais pas — montre-moi où" as an answer but nothing says what happens after showing — the run either re-asks by improvisation or writes with the rule undecided, and the new headline rule ("⚠ on any pas voulu, ✔ otherwise") prints ✔ over it | review: three-zone-conclusion-spec-arch |  |  | Open |
...
| B-043 | 2026-08-19 | ✨ improvement | lo | 4100 | Projection freshness — `esq state` reports `roadmap.stale` (and epic staleness where apt): live backlog status per covered ID beside what the projection says, deterministic freshness evidence (source hash or timestamp), never rewriting ROADMAP.md; `/esq:status` distinguishes projected order from stale status text | manual (external review 2026-08-19 — ROADMAP.md showed B-028 Open after the backlog marked it Done) | aug-18-improv |  | Open |
...
| B-047 | 2026-08-19 | ☑️ todo | med | 4000 | Probe whether a skill `model:` pin is honored when the command is typed into a running *interactive* session — the probe's `typed /skill` row is headless `claude -p "/skill"` (honored), while B-026 (sonnet pin → opus ×23) and 27/27 direct telemetry rows of 2026-08-19 (`model: opus` orchestrators → `claude-fable-5`) say a typed command in a running session keeps the session model; one row (or a documented interactive protocol) so README's pin story separates the two paths on evidence; answered inside B-074's versioned compatibility/evidence suite rather than as a second standalone mechanism | plan: relay-worker-model-separation · scoped into B-074 2026-08-21 | aug-18-improv |  | Open |
...
| B-050 | 2026-08-19 | ☑️ todo | med? | 4800 | A plan whose verification proves a chain through headless `claude -p` must carry `--permission-mode bypassPermissions` (no prompt can answer a write in `-p`) and must not export `CLAUDE_PLUGIN_DATA` (the hook ignores it, the CLI does not) — both cost a billed rerun in relay-worker-model-separation Phase 4; a line in `/esq:plan`'s verification-authoring guidance | build: relay-worker-model-separation Phase 4 | aug-18-improv |  | Open |
...
| B-056 | 2026-08-20 | ☑️ todo | med? | 5000 | `/esq:check` and `/esq:review` each carry a lane-bearing preflight step and resolved-target line (lean-assurance-lane Phase 4) that no `docs/CONFORMANCE.md` scenario pins — R-05 covers `converge` only, so a packaging migration could drop both finders' lane prose and check 24 would stay green, leaving a user unable to tell which lane a finder was bought for; it is a fourth scenario and its own decision, which is why Phase 5 did not fold it in | build: lean-assurance-lane Phase 5 | aug-18-improv |  | Open |
| B-057 | 2026-08-20 | 🐛 bug | med? | 600 | `esq lane stats` counts findings by tier from the corrective-brief files still on disk, but `/esq:fix` strips applied 🟢 items from a brief and deletes one whose items all applied — so the 🟢 column reads near-zero by construction (0 across this repo's five surviving briefs) and anyone comparing lanes by finding volume is misled in the direction that flatters the lean lane; the accurate source is each brief at its `brief(fixes):` commit, which is the per-plan history walk D-lane-stats-measures-from-ledgers-not-history rules out, so this is that bound being re-decided rather than a fix | build: lean-assurance-lane Phase 6 · Dropped: its whole subject was `esq lane stats`' finding-tier column, and that command retired with the assurance lanes on 2026-09-22 — there is no consumer left to mislead. Nothing here was a defect in the corrective loop itself. | aug-18-improv |  | Dropped |
...
| B-070 | 2026-08-21 | ✨ improvement | hi | 3400 | An agent's bill and wall-clock are both round trips × context size — 604 agents, 24,403 tool-issuing round trips, 82.8% singleton, 109,825 cache-read tokens per round trip (2.748 G). Narrowed to the re-read half: **757 round trips lost to re-reading a file the same esq run already read** — 4.8/run, 16.5% of tool-issuing turns, ~42s off a 7m16s `esq:build` median, and the repeat targets are esquisse's own ledgers. Batching is B-071; the `TaskCreate`/`TaskUpdate` row is void (esq agents make zero Task calls) and the batched-reads row reprices 4,987 → 318 | manual (esq telemetry summary + 621 agent transcripts, corrected 2026-08-21) · Planned by read-once-and-see-round-trips · measurement delivered as B-151, see detail | aug-18-improv |  | Planned |
...
| B-075 | 2026-08-21 | ✨ improvement | med | 4200 | Parked work has no honest representation — B-001 reads `Open` while parked on a cell-capable Sheets connector that does not exist, and B-006 reads `Planned` with a plan not started, so `/esq:roadmap` rolls it up as `in flight` against D-park-the-port-while-esq-is-load-bearing ("planned and parked, not started"). Give externally blocked or deliberately shelved work one durable representation — a `Parked` value in the closed Status set, or an equivalent convention every existing reader parses — carrying why it is parked, the event that unblocks it, and optionally when to reconsider. Decide the mechanism against the closed Status set, `esq backlog set-status`, `esq validate` and the roadmap rollup. Narrow: not a scheduler, not project-management software | manual (roadmap correction 2026-08-21) |  |  | Open |
| B-076 | 2026-08-21 | ✨ improvement | med? | 200 | A UI phase's visual evidence does not outlive the session — `/esq:build`'s manual-step procedure records what the driver returns (screenshot, DOM text, rendered value) and reports it in the final message, but the execution log keeps only the instrument's name, so `/esq:check` and `/esq:review` cannot inspect the result the phase claimed to observe and a `(manual)` step ends up verified on prose. Keep a lightweight durable reference per **named UI state** — an artifact path, a stored capture reference or equivalent — written into the log entry through `esq plan append-log` so the finders can read it. Conditional on phases that actually render UI; explicitly not a general screenshot platform, and explicitly not a new command — beauty stays an evidence problem, and ui/grill/plan already own the design stages | manual (roadmap correction 2026-08-21) |  |  | Open |
...
| B-078 | 2026-08-21 | ⚠️ debt | med? | 3300 | The same SIGPIPE-under-`pipefail` shape as B-068 survives with `head` as the early-exiting reader — `scripts/worktree.sh:59`, `:93`, `scripts/check-conformance.sh:36`, `:38` and `scripts/audit.sh:534`. None is a live defect today: every one of the five discards the pipeline status into a `$(…)` assignment or a function whose callers ignore it, so a 141 goes nowhere. But `worktree.sh` runs under `set -euo pipefail`, where a status that starts being consumed aborts the script, and `check-sigpipe-shape.sh` deliberately covers `grep -q` only — widening it to `head` would redden the audit on all five. Decide per site: rewrite to a here-string, or record why the status is provably unread | plan: pipefail-sigpipe-grep-sites (scoped out) | aug-18-improv |  | Open |
| B-079 | 2026-08-21 | ✨ improvement | hi? | 900 | `/esq:roadmap`'s `→ Or: /esq:advance` offer and `/esq:status`'s equivalent line still emit the bare, whole-horizon form now that `/esq:advance <slug\|B-NNN>` exists — so the one person actually looking at the queue, at the moment they are looking at a single entry, never learns the walk can be aimed and pays for the full horizon or nothing | build: advance-entry-scoping Phase 1 |  |  | Open |
...
| B-085 | 2026-08-22 | ✨ improvement | hi | 1000 | A `build` worker that runs out of independent work has no cheap way to wait for a finite check — 123 excess status round trips in 40 of 136 eligible agents, 16.1 M cache-read (frozen baseline `--through 2026-08-22T12:00:00Z`); `fix` and `work` measure zero, so this is build's rule alone. Blocked on `runtime-evals` (B-032) for safety and `outcome-instrumentation` (B-073) for the value half | manual (measure-wait-cost.mjs v2 + 2.1.239 harness probe, 2026-08-22) · plan wait-once-and-a-named-payload superseded before any phase ran · Planned by background-task-lifecycle | aug-18-improv |  | Planned |
...
| B-087 | 2026-08-22 | 🐛 bug | med | 1100 | A background task stopped by PID keeps a harness registration alive; its `killed` notification arrived 51 min later and resurrected the concluded worker for a full-context turn (2 of 136 agents in the frozen baseline). The parent then reports agent lifetime as the phase's elapsed — 23m shown as 1h13m22s — though the worker's own figure was right. Why the notification was delayed is inference until probed | manual (transcript forensics on agent a70e8db3, CC 2.1.239, 2026-08-22) · Planned by background-task-lifecycle | aug-18-improv |  | Planned |
...
| B-093 | 2026-08-30 | 🐛 bug | hi? | 3700 | `/esq:build`'s "Escalate the lane when the phase finds a trigger the plan missed" section did not fire on a trust boundary in two billed headless runs (Claude Code 2.1.251, 2026-08-30, `tests/journeys/fixtures/billed/escalation.jsonl`): given a `direct` plan recording low/low and one task persisting the caller's API token to `.tokens.json`, build wrote the module, completed Phase 1 normally and recorded no escalation — `esq lane` on the file still resolves `risk=low`, `source=plan`, so the next orchestrator reads an unraised lane. The corroboration regex `/escalat/i` was false on the second run: the word never appeared in the stream at all. U-06's runtime evidence (journey 3 of B-032) cannot be minted until this fires | build: runtime-evals-golden-journeys Phase 3 · Planned by build-escalation-actually-fires · Dropped: the behavior it waited on was removed, not fixed: /esq:build's assurance-escalation section, the lane it raised and append-log's escalation field all retired on 2026-09-22, so there is nothing for the parked escalation journey to unpark into. Its judge, overlay and billed red capture retire with it. |  |  | Dropped |
| B-094 | 2026-08-30 | ⚠️ debt | med? | 3500 | Each `tests/journeys/fixtures/mutated/<branch>.jsonl` is a full copy of `capture.jsonl` with one field changed, so every journey added re-derives every mutation that came before it — N files each carrying N journeys. The original Phase 3 committed a four-branch `ROWS` against two-branch fixtures and reddened main this way; the bijection invariant landed in Phase 3 catches a forgotten regeneration, but the churn stays. Shape a mutation as a one-journey overlay applied onto the good capture in-process, so adding a journey never touches an existing mutated file, and `--parse --only <branch>` replays it | plan: runtime-evals-golden-journeys re-plan (cut from Phase 3 as churn, not risk) | aug-18-improv |  | Open |
| B-095 | 2026-08-31 | ✨ improvement | med? | 3800 | Expanded runtime-eval coverage on the journey runner B-032 shipped — the behaviors cut from B-032's description when it was reduced to its delivered v1 on 2026-08-31. Wanted, each as one journey on `scripts/smoke-journeys.mjs` with an artifact-only judge, a branch-local red mutation and a `docs/CONFORMANCE.md` evidence line naming the branch it actually bought: **U-05** — an orchestrator asserts the worker's model and stops before its next spawn on a mismatch; **U-03** — unattended execution grants exactly one reconciliation pass to an unclassified phase and hands off attended (the relay-a-reported-failure branch is a second journey, not the same one); **U-04** — a decision authorizes exactly the offered `do:` and is not asked twice, which needs a two-run journey the harness does not yet support. Plus the broad ambitions never scoped to a journey: no unauthorized mutation before a mandate; each lean-assurance classification routes exactly once; malformed plan or backlog state degrades locally; corrective loops terminate within their documented bounds; expensive workflows respect declared run or token budgets; orchestrators never spawn parallel workers into one working tree; a UI phase observes every named state or pauses rather than self-certifying from tests. Design for the first three is preserved at commit **cd3485f** (the removed Phases 4–6) — read it before re-planning. **This row follows the verification-collection objective, it does not block it:** the journeys a workflow change needs are prerequisites on that change's own row (B-085/B-087 already carry theirs), not a coverage campaign queued ahead of it. B-094 (mutations as one-journey overlays) is worth doing first — every journey added here otherwise re-derives every existing mutation | plan: runtime-evals-golden-journeys (scope cut 2026-08-31; design preserved at cd3485f) | aug-18-improv |  | Open |
| B-096 | 2026-08-31 | ✨ improvement | med? | 4300 | Nothing in the set closes the loop from a correction the user makes twice to a check that fails — the doctrine exists (`mechanize-repeated-corrections`, `docs/AUDIT.md`) but the observation, the counting and the promotion are all manual, so a rule corrected in a session nobody harvests is lost. `/esq:harvest` does decision archaeology and `/esq:sweep` reconciles the backlog; neither watches a correction happen. Prior art: ECC's `continuous-learning-v2` (hooks → `observations.jsonl` → background Haiku observer → atomic `{trigger, action, confidence 0.3–0.9, domain, evidence}` instincts, project-scoped by repo hash, promoted global at 2+ projects, clustered into skills by `/evolve`). **Its shape is wrong for esq on three counts and must not be copied**: it persists prompts and tool inputs (violates `plugin-runtime`'s hooks-store-numbers-never-content), it promotes into always-loaded context (the cost class `shared:read-once` exists to forbid), and a confidence score is not a justification (CLAUDE.md cost question 4 — automating a manual step inherits the written rule and none of the unwritten one). Wanted instead: the hook counts a correction signal content-free; a user-typed command proposes the promotion with the evidence; and the promotion lands as a `check-*.sh` with its fault injection, never as a rule to re-read | manual (external comparison 2026-08-31: affaan-m/ECC `continuous-learning-v2`) |  |  | Open |
...
| B-107 | 2026-09-02 | 🐛 bug | med? | 1200 | `/esq:converge`'s landing gate re-runs every phase's `(auto)` step with no equivalent of build's second exception, so a branch whose plan carries an unrunnable command cannot land even though the phase that wrote it completed | build: auto-step-that-cannot-run Phase 2 | aug-18-improv |  | Open |
...
| B-109 | 2026-09-02 | 🐛 bug | med? | 1400 | `tests/cli/merge.test.mjs` is gated by no audit check — check 30 names `esq.test.mjs`, `branch.test.mjs` and `ship.test.mjs` explicitly — so a red merge suite, covering the only CLI code that writes to git, still leaves `./scripts/audit.sh` at exit 0 | build: harness-evidence-freshness Phase 2 · Done by retire-manual-cli-fallbacks | aug-18-improv |  | Done |
| B-110 | 2026-09-02 | 🐛 bug | med? | 1500 | `scripts/check-sharedblocks.sh` cannot be pointed at `plugin/skills` — it globs `<dir>/*.md` and exits 2 on the plugin tree, so the shared blocks are only ever compared in `commands/esq`; when the legacy rollback copies go, nothing checks the source of truth | build: harness-evidence-freshness Phase 3 · Planned by retire-legacy-rollback-path · Done by retire-legacy-rollback-path | aug-18-improv |  | Done |
...
| B-113 | 2026-09-03 | 💡 idea | lo? | 2500 | `scripts/audit.sh`'s green line for check 48 enumerates only the registry cases and never mentions the relay-model cases, so a reader auditing coverage from the audit output alone concludes a re-pinned relay is unguarded | build: relay-inherits-the-session-model Phase 2 | aug-18-improv |  | Open |
| B-114 | 2026-09-03 | 🐛 bug | med? | 2100 | Nothing mechanizes README § Model recommendations against the skills' actual `model:` frontmatter — check 47's assertion 4 reads only the `M-` measurement IDs — so a flipped pin leaves the table telling a reader the wrong model to set their session to; corrected by hand twice now (2026-08-19 relay pin, 2026-09-03 `inherit`) | review: relay-inherits-the-session-model | aug-18-improv |  | Open |
...
| B-117 | 2026-09-04 | ☑️ task | med? | 1800 | Check 42's five branch-ownership assertions have no fault injection — it is inline in `audit.sh` with no paired `scripts/test-*.sh`, so a loosened needle or a broken `branch_holds` stops guarding silently | build: origin-branch-owns-landing Phase 4 · Dropped: check 42, the prose duplicate of the branch-ownership contract, was removed on 2026-09-22 — there is no longer an uninjected assertion to inject. The verdict itself is the CLI's and is covered by tests/cli/branch.test.mjs. |  |  | Dropped |
| B-118 | 2026-09-04 | 🐛 bug | med? | 3900 | No runtime journey observes a shipping unit landing on its recorded origin — `esq merge land --plan` is unit-tested against synthetic origins and `/esq:plan`'s branch cut is asserted only as skill prose by check 42, so a header shape the skill writes and `land` cannot parse passes every mechanical gate | check: origin-branch-owns-landing |  |  | Open |
...
| B-122 | 2026-09-05 | ✨ improvement | lo? | 2400 | Nothing checks that a script under `scripts/` appears in README's Operations table — check 4 holds the `/esq:` command tables exhaustive but the scripts table is unguarded, so a contributor adding a script (as `outcome-join.mjs` just did) can ship it undiscoverable and the audit stays green | build: outcome-linked-workflow-measurement Phase 4 |  |  | Open |
...
| B-129 | 2026-09-06 | 🐛 bug | hi? | 800 | Bare `/esq:roadmap` reads "plan complete + backlog row still `Planned`" as stale work to close and recommends `/esq:sweep`, ignoring an explicit post-plan completion condition recorded on the item — it proposed closing B-121 while the live baseline gate exits 3 at 56% coverage with two conditions failing, and acting on that would prematurely release B-071/B-044/B-085/B-087 and invalidate the pre-change baseline; roadmap (and any command that proposes closure) must read conditional closeability before recommending it | manual (roadmap refresh 2026-09-06, cc185da) |  |  | Open |
| B-130 | 2026-09-07 | ✨ improvement | lo? | 5200 | `release-local.sh` only bumps a patch — a minor or major means hand-editing both manifests, which is exactly how the two come to disagree (the failure check 51's agreement half exists for); named in the plan's own Risks as worth a row rather than a flag nobody asked for | build: local-plugin-release-workflow Phase 2 |  |  | Open |
| B-131 | 2026-09-07 | ⚠️ debt | med? | 5600 | `plugin/skills/plan/SKILL.md` sits at 499 lines against `check-plugin.sh`'s 500-line entrypoint cap, and a non-build skill has no sanctioned split — `check-skill-parity.sh` compares the legacy mirror byte-for-byte against the whole skill body — so the next addition to `/esq:plan` reds the audit with no remedy but deleting prose someone wrote on purpose | build: fewer-round-trips-per-run Phase 2 · Planned by retire-legacy-rollback-path · Done by retire-legacy-rollback-path |  |  | Done |
...
| B-133 | 2026-09-09 | ☑️ todo | med? | 3600 | `scripts/smoke-journeys.mjs` offers no supported way to add a journey to the registry: promotion needs a whole-registry re-buy (~$3.75) because the aggregate must record one Claude Code version, and neither the usage text nor any refusal says so before the money is spent — the next contributor appends their `--only` capture, as this plan's Phase 3 did, and reds three audit checks | build: 2026-09-08-background-task-lifecycle Phase 3 |  |  | Open |
...
| B-141 | 2026-09-09 | ⚠️ debt | med? | 2300 | Nothing mechanically checks that a decision a newer one reverses is marked `Superseded`, so `docs/DECISIONS.md` can hold two contradicting `Active` rows with the audit green — `audit.sh` reads the file only as an artifact path (line 50) and no check parses its statuses | check: restore-converge-and-clean-handoffs |  |  | Open |
...
| B-149 | 2026-09-11 | ☑️ todo | med? | 700 | Retire the corrective briefs `/esq:plan` planned before it retired them itself — nine `*-fixes.brief.md` on disk already have a plan of their slug (e.g. `2026-09-03-harness-evidence-freshness-fixes.brief.md` beside `…-fixes-2.md`), and `unit.findings` keeps refusing each of their units' landings; each belongs to its own shipping unit, so it is not this plan's to delete | plan: landing-is-its-own-command-fixes |  |  | Open |
...
| B-160 | 2026-09-15 | ✨ improvement | lo? | 5300 | check-refload.sh pins only entrypoint call sites, so the two load reminders manual-verification.md carries for the failure procedure can be deleted with nothing reddening — the obligation itself stays pinned in build's entrypoint, so this is thin coverage rather than an open route | build: build-loads-failure-on-demand Phase 1 |  |  | Open |
...
| B-162 | 2026-09-15 | ✨ improvement | lo? | 5400 | nothing mechanically checks that /esq:build's preflight prose names the keys `esq next-phase --context` actually emits — parity holds the two build carriers together and tests/cli/esq.test.mjs holds the response shape, but a renamed context key would leave the skill describing fields the CLI no longer returns with nothing reddening; the CLI-unavailable fallback still works, so this is thin coverage rather than an open route | build: build-reads-its-context-once Phase 1 |  |  | Open |
| B-163 | 2026-09-15 | ✨ improvement | lo? | 1300 | a plan's `(auto)` step whose only inline-code span before the em dash is a file path resolves to that path as its command — `extractAutoCommand` (`plugin/lib/markdown.mjs:267`) takes the single span between the marker and the dash whatever it holds, so `esq gate verify` hands `/esq:land` and `/esq:converge` a plan path to execute and the landing gate reds on something that was never a command; observed while authoring build-reads-its-context-once Phase 1 and corrected in the plan, but nothing warns an author | observed: build-reads-its-context-once Phase 1 |  |  | Open |
...
| B-165 | 2026-09-16 | ⚠️ debt | med? | 5100 | build's failure route says "report in the three zones" but never loads the conclusion block that defines them | observed: bookkeeping-loads-on-demand |  |  | Open |
...
| B-169 | 2026-09-19 | ✨ improvement | hi | 300 | esq blocks on implementation-detail questions instead of deciding and shipping | work | esq-decides-implementation-detail |  | Open |
...
| B-171 | 2026-09-20 | 🐛 bug | med? | 2000 | The hooks tmpdir-leak assertion excludes only `esq-` prefixed entries, so running every suite in one node --test process reds it on `worktree-landing`'s deliberate `esq wt-` fixture | observed: every-open-item-is-ranked Phase 1 |  |  | Open |
| B-172 | 2026-09-20 | ✨ improvement | lo? | 2600 | esq backlog add files a row with no rank, so every row the cycle files lands unplaced until someone re-ranks | build: every-open-item-is-ranked Phase 3 |  |  | Open |
...
| B-179 | 2026-09-21 | debt | hi |  | The corrective machinery can trap a trivial fix with no exit: a two-word prose correction in a shipped plan's prospective section was refused by every sanctioned path — /esq:fix (prospective plan sections are outside its write set), /esq:plan (esq brief depth exhausted, 3 vs bound 2) and esq plan abandon (refuses a plan whose phases all completed) — so a review brief that blocks landing via unit.findings had no command able to dispose of it and the fix had to be made by hand outside any phase. Either a finder must not tag 🟢 an item no fixer may write, or a depth-exhausted brief needs a sanctioned disposal that is not a third generation. | observed: /esq:converge on observation-path-before-manual-fixes-fixes, 2026-09-21 |  |  | Open |
| B-180 | 2026-09-21 | debt | med? |  | `esq gate verify` treats every `docs/plans/**` path as command-owned lifecycle (`GATE_LEDGERS` / the harmless set, `plugin/lib/cli.mjs:2819`), so a live `(auto)` step whose *input* is a plan file is marked `reuse` after that file changes. Observed at dc380db: 31d9a99 added two new `2ac72b6` occurrences to `docs/plans/2026-09-18-observation-path-before-manual.md` after the Phase 1 `**Verified:**` hash 14919c8, and both lexical negative controls that grep that exact file (`grep -n 'diff --stat 2ac72b6 -- plugin/skills/plan/SKILL.md[^ ]' ...` and `grep -n 'SKILL.md. is byte-unchanged from .2ac72b6' ...`) still resolve `reuse`, not `run`. They happen to still pass here, so nothing red landed — but the exemption is unconditional: any post-verification prose edit to a plan can falsify a grep-over-plan-file criterion and `/esq:land` will ship on the stale PASS with no symptom. Wanted: a path is harmless only when no recorded command reads it — the inverse of B-155 (over-invalidation), not covered by it. | review: observation-path-before-manual-fixes-fixes |  |  | Open |
