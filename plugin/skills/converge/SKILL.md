...
The `/clear` stops ending `/esq:check`, `/esq:review` and `/esq:fix` are `[context]`-class; **a subagent is a clean session**, so spawning each step dissolves exactly those stops. Every other stop is a decision owed to the user, and you honor all of them — see "Gates you honor".
...
`/esq:check` remains a user-requested diagnosis. Never follow a worker's `→ Next`, repeat a step, or add a re-review after fix. Brief entry starts at step 2 under the provenance rules in Preflight.
...
Do not add a step to close these gaps.

## The invariant — no decisions about cost, at any point

**The itinerary is fixed before the run starts and is never re-decided inside it.** Nothing a subagent returns lengthens, shortens or reorders it — not a scarier-than-planned review, not a fixer that escalated everything to 🟡, not a `→ Next` pointing past step 2. The user overrides it by invoking a finder directly, which you never refuse.
...
1. **Resolve the argument.**
   - A `*-fixes.brief.md` path → the run **enters at the fix step**, one subagent instead of two.
...
2. **Check the branch owns this plan — before the first spawn.** Run `esq branch check <plan-path>` once, here, on the plan path step 1 resolved; it alone decides branch ownership. Route off `refuse`, never off the prose:
   - **`refuse: false`** → carry the verdict into step 7's announce line — `branch <name>` for `ok`, `branch unrecorded — legacy plan` for a plan predating the field — and continue.
   - **`refuse: true`** → STOP here, before any spawn or write, with the option set in "Refuse a branch this plan does not own". Never switch a branch yourself, and never edit the plan header to make this run proceed.
...
3. **Classify the phases from the headings:** `grep -n -E '^#{1,3} ' <plan-path>` (a subset of `/esq:autopilot`'s `shared:log-skeleton` grep, which stays the authority on its meaning; open no other command file). The `### Phase N —` headings above `## Execution log` are the phases; those inside the log span are their entries, each classified by its heading text: `completed`, `⏸ paused`, or no entry. Read no entry body.
4. **Refuse an unfinished plan.** If any phase is `⏸` or has no log entry, STOP. → Next is `/esq:build <plan-path>` (a `⏸` pause) or `/esq:autopilot <plan-path>` (phases remaining).
5. **Fetch tools:** `ToolSearch "select:Agent,AskUserQuestion,PushNotification"`. Without `AskUserQuestion` a 🔴 degrades to a printed block and a stop. If `Agent` is unavailable, STOP — say unattended orchestration isn't possible in this session and the user should run `/esq:check` themselves. Never run the steps yourself.
6. **Record the starting `HEAD` short hash**, and snapshot the current set of `docs/plans/*-fixes.brief.md` paths.
7. **Announce the resolved target:**
   <!-- announce:start -->
...
Reached only from preflight step 2, on `refuse: true`. This is a terminal **authority** stop: nothing spawned, nothing written. Converge never picks a branch, never runs `git switch`, and never edits the plan's `**Branch:**` field (`/esq:plan`'s, single-writer among commands). The user's own edit of their plan header is the escape — option A's second step.

Hand the decision back in this shape and no other: `reason` is the opener, `branch` and `recorded` fill the `git switch` lines, `owners[].file` names the conflict, and the plan's filename slug is the fresh-branch name.
...
Each verdict shifts the wording, never the shape:

- **`mismatch`** — the plan recorded a branch, HEAD is on another. Name both. All three options apply; lean B whenever `recorded` still exists as a ref.
- **`owned-elsewhere`** — a *different* plan recorded this branch. Name that plan's path as a fact. Drop options B *and* C and lean A — give both of its steps, since neither the switch nor the header edit alone clears the gate.
- **`detached`** — HEAD is on no branch. Offer only A, plus returning to the previous branch (`git switch -`).

Then stop. Do not re-run the check, do not spawn the itinerary's first step, and do not fall through to the announce line.
...
1. Re-list `docs/plans/*-fixes.brief.md`. The brief is the path new since your snapshot, or the one whose mtime moved; if two candidates are ambiguous (finders append `-2`, `-3`), cross-check against the finder's `brief(fixes): <slug>` commit (`git show --stat`). Update your snapshot.
2. **No new or modified brief** → the clean outcome.
3. Otherwise read the brief and count the items under each of `## 🟢 Fix now (safe)`, `## 🟡 Needs a plan`, and `## 🔴 Needs your decision`.

<!-- brief-routing:start -->
**🔴 outranks every other tier.** Any 🔴 item present → settle it before any fix runs; it may decide whether the 🟢 items are worth applying at all. Never apply the greens first, and never judge a red "independent" of them. Put it to the user as its option set, and let their answer land before the fix step begins.
<!-- brief-routing:end -->

Then: any 🟢 → run the fix step against this brief. No 🟢 → skip the fix and go to the report.

**After the fix (step 2):** a fixer that escalated items to 🟡 is a result, not a gate. Read what it did:

1. `git log --oneline <hash-before-this-step>..HEAD` — the `fix(...)` commits are the applied items. Record the hashes.
2. Re-read the brief. Deleted → everything was applied. Present → its 🟡/🔴 are what survived, including anything downgraded mid-run.
...
Halt the itinerary immediately and relay the reason **verbatim**.
...
- A `/esq:check` verdict of the form "built as planned, but it doesn't answer the question" — a scope finding; re-planning is `/esq:plan`'s, never yours.
...
**The split is whether an answer exists that you could act on**, not severity. Never soften a gate, summarize away the question, or judge that it "looks minor".
...
A 🔴 already in a brief arrives in that shape, so copy it across untouched. **You never author an option or a leaning yourself.** A brief red carried as a bare question: say so, quote it, and make → Next the attended re-run of the step that produced it — never your reconstruction. **A report with no option set is a diagnosis, relayed as one:** the agent's diagnosis and exact action verbatim, the failing output under `Evidence:`, no option, leaning or decision line around it; → Next is the agent's named action. Ask the agent again for an option set only when its own report names a missing user authority and offers no options.
...
**A relayed decision is *selected*, not applied.** On `route: relay` nothing was spawned or changed, so record the answer: write one line into that 🔴 item in `<brief-path>`, indented under it —

> `- Selected: **<letter> · <label>** — direct invocation required — run: ``<the chosen option's do: string, verbatim>`` — recorded <YYYY-MM-DD>`

— then commit the brief alone as `brief(selected): <slug> — <the decision>, <the chosen option's label>`, print the command flush-left, and stop the itinerary there. **The 🔴 stays** until its effect exists.

**On resume, the recorded selection is the answer — never put the same decision twice.** A 🔴 carrying a `Selected:` line is already decided: do not offer it, ask about it, or spawn on it. An unclassifiable pick follows its history below: the original is never routed again. Call `esq apply route '<the recorded do:>'` once and route on `applied`:

- **`true`** — strike that one 🔴 exactly as the apply agent would have, commit it alone as `brief(decided): <slug> — <the decision>, <the chosen option's label>`, and resume the itinerary at the step after the one that gated. No finder re-runs.
- **`false`** — print the command flush-left again and stop: no question, no spawn, no edit to the brief.
- **`null`** — `confirm` says why. Print the command again, say in one line that this run cannot observe whether it ran, and stop with the itinerary's next step as `→ Next`. The 🔴 and its `Selected:` line stay, and no step proceeds on a decision that was only displayed.
...
**On resume, act on the last line, never on an earlier one.** `Repair approved #<n>` → route #<n> once; when it routes `relay`, branch on its `applied` exactly as for a recorded selection above. `Paused` or `Repair failed` → propose again, naming what it supersedes; an older approval runs nothing unless re-approved. `Selected` → step 2.

**Then resume the itinerary at the step that gated** — a 🔴 found in step 1's brief resumes at step 2, never step 1. If the apply agent reported it could not apply or verify the change, do not resume — report that, with any edits it left, and stop.
...
🟡 items are **not** a gate — they are the expected terminal state, go in the report, and become the `→ Next`.

## Ready to land — the clean completion

**A clean completion is ready to land, and this command never merges.** Landing is `/esq:land`, which re-checks every durable prerequisite itself. This section decides only whether this run was clean, and records it.

**The clean bar, cheapest first.** Stop at the first that fails and report `○ not ready` with the command that clears it:

1. **No gate is left open, and no 🟢, 🟡 or 🔴 survives in any brief this run read.** Both are already in hand from the step classifications — do not re-read a brief.
2. **`unit.open` is still empty.** The fix steps may have filed a 🐛/⚠️ row; re-read it with `esq branch check <plan-path>` again, once, since `HEAD` and the ledger have moved since preflight. A row still standing means not ready.
3. **A review has actually read this unit.** Either step 1 ran `/esq:review`, or preflight's `finder` was `review`. A `check` brief, or one whose `Source:` names no finder, fails: report `○ not ready — fixes applied, no review has read this unit`, make `→ Next` `/esq:review <plan-path>`, and stop here: **nothing below runs.** This adds no step.
...
A ready result may still be refused by `/esq:land` (unit completeness, promised rows, verification, destination). Never require or perform a spec/arch refresh.
...
Emit each zone-3 step line **the moment that step resolves**, not at the end, so a session that dies mid-run leaves a trace.
...
Zone 2 carries a gate **only while it is still unresolved** — unanswered, unappliable, or raised with no `AskUserQuestion` available. An answered gate is only its zone-3 line recording the pick; never reprint its options. A resolved gate may carry an indented `Evidence:` (≤10 lines) only when its outcome line cannot state what happened.

**The completion is one zone-3 line, and it is never absent**, in one of two forms:
...
`✔ ready to land` carries the commit the coverage names. `○ not ready` carries the first failing check of the clean bar and **always** its command. Neither line is a landing; this command never reports `landed`.

`closed` names the fixer that closed the row; you closed nothing yourself.

Compute `→ Next`:
- Stopped on an **unresolved** 🔴 → `"<N> decision(s) above — run the `do:` of the option you pick on each, then: /esq:converge <plan-path>"`. Name no option as chosen. Never `"answer the 🔴 item(s)"`.
- A decision was answered and applied, and the run then completed → treat it as any clean run.
- Stopped on a **relayed** gate — the chosen option's `do:` or its approved repair routed `relay`, or a resume whose `applied` came back `false` → the relayed command first, exactly as written, then `/esq:converge <brief-path>`. On `applied: null`, name the command and then the itinerary's next step instead.
...
- Nothing remains and the clean bar stopped the completion → the command that clears it: `/esq:backlog` for a `unit.open` row still standing.
- Nothing remains and the completion is recorded → `/clear`, then `/esq:land <plan-path>`. No spec or arch refresh is owed first, and this run runs neither.
...
- **Orchestrate only — you write no code** and never `docs/BACKLOG.md`. Your writes are listed under "Deterministic CLI"; every other mutation is a subagent's.
- **Never merge.** Not through `esq merge land`, not by hand, not "because the run was clean". Never stage, never switch a branch, never push.
- **Never apply, promote, or reinterpret a 🟡 or 🔴 on your own.** Applying a 🔴's option the *user* picked is their judgment; a 🟡 has no options to pick.
- **Never re-run a step** — a failure is a finding, not a flake; resuming continues *forward* — and **never extend the itinerary past step 2** (no second review, `/esq:check`, `/esq:plan`, `/esq:build`, `/esq:arch` or `/esq:spec`). You recommend and stop.
- **At most two subagents** — one when entered from a brief — strictly sequential, never concurrent, plus one apply agent per decision whose approved action routes `apply`. State the bound up front and never exceed it.
- **A 🔴 is resolved once per gate by the user's pick, never unanswered, never with an option you wrote.** Never answer one on the user's behalf, and never pick an option from an `AskUserQuestion` a subagent raised. Relaying a brief's or a step agent's leaning is not answering; forming your own, or acting on a prompt nobody answered, is.
- **Report the bill** — steps, commits, elapsed time — per step as it resolves, and again at the end even on an early stop. If a step failed, the report says so first.
