...
6. **No plan/brief action, roadmap exists** → use the entry the user is discussing (argument or conversation, resolved through `roadmap.entries` by slug or covered reference), otherwise the top Now entry: Open item → `/esq:work B-N`; epic → `/esq:epic <slug>`. Name slug/whyNow. A blocked entry targets its known blocker; an empty head or unknown target/facts → `/esq:roadmap` exposes the queue. For a discussed Now entry with eligible Open work, offer `→ Or: /esq:advance <slug>` (only that entry, stops before build), even for one item. Never offer advance for an entry outside Now or widen a failed lookup. With no particular entry selected, **at least two still-Open items in Now** → offer `→ Or: /esq:advance`; an explicit whole-Now request also keeps this bare command. Never invent the count; advance retains its existing dependency and plan checks.
