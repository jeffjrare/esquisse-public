...
2. **Resolve the planning input.** Use the user's task description when given. Otherwise run `esq brief pending` and read `selected` in full: its scope, decisions, constraints and deferred work are the input. Announce its path and the other `pending` briefs this run will not plan. Derive the slug by stripping `.brief.md` and the leading date; for a direct task, use 3–5 lowercase words joined with hyphens.
...
3. **Check `docs/BACKLOG.md` if present.** Match `Open`/`Needs-decision` rows by the task's nouns in their Summary; read a detail section only after its row matches. Include only relevant items in `## Open questions` or `## Risks`; resolve blockers before planning proceeds.
4. **Check for an epic** when `docs/epics/` exists. Use the epic the user or brief names, or the single `Active` epic whose `## Scope` clearly covers the task. Otherwise omit it; never force a fit or create an epic here. For work exceeding one plan, mention `/esq:epic new <title>` if useful. Keep the chosen slug for recording.
...
Keep phases sequential and sized for one fresh execution session each. Prefer a first usable path
...
If your recommendation depends on a choice the ask-altitude block below marks **theirs**, fetch `AskUserQuestion` with `ToolSearch "select:AskUserQuestion"` when available and resolve it before writing the plan. Present each option with its tradeoff as the description. One question maximum.

Anything else, make the call and note the reasoning in the plan.

<!-- ask-altitude:start -->
**Resolve questions from the repo, then outside research, before asking the user.** Read the mandate, code, approved plan and Active decisions. Engineering and UI choices within those goals and budget are yours: decide and carry their rationale into the plan. Significant choices are recorded when the contract is written.
...
the plan look small. States belong in the phase exposing the action.

Review the prepared plan once and correct these yourself:
...
## Deliver the prepared plan

For a response-only proposal, use the requested scope and length and stop there.
Otherwise load `${CLAUDE_SKILL_DIR}/references/record-contract.md` to write the single
plan file, record significant decisions and picked-up rows, then commit and report.

## Constraints

- Do NOT make code changes outside the plan file — the ledgers aside, the one other file you touch is the corrective brief you planned, retired through the contract reference
