...
6. Read `CLAUDE.md` at project root if present — in step 3's turn, since neither waits on the other. **The very next turn reads the phase's working set** (see "Execute the phase"): every file the phase's tasks name, their tests, and the project skills or docs the phase will need, together in one turn. Investigation that follows reads only what that set lacked, a search chained with the slice it leads to.
...
Use preflight's response; no second query or file read. `context.phases[].status` is `completed`, `paused` (any `⏸`), or `null` (no entry). Unlogged does not prove unexecuted; reconciliation below checks that.
...
1. **Paused phase takes priority.** If any phase has a `⏸` entry, do NOT start a new phase. Resolve it first, and which resolution depends on the clause:
   - `⏸ awaiting manual verification` → "Resolve a paused phase", and the "Observe the `(manual)` steps" procedure it runs, are both in `${CLAUDE_SKILL_DIR}/references/manual-verification.md`: you MUST load it before you resolve the pause, and a run that resolves one never self-certifies the steps instead.
   - `⏸ blocked on an open same-unit defect` → "Resolve a blocked phase" is the only path out: it re-executes no task and ends in a CLI write, never a hand-edited header. That section is `${CLAUDE_SKILL_DIR}/references/resolve-a-blocked-phase.md`: load it when this branch sends you there, and never for a phase that is not blocked.
...
Two loads are decided here, from the phase just identified, and **whichever apply go out together in one turn**.

- **This phase renders user-facing UI** → `${CLAUDE_SKILL_DIR}/references/ui.md`.
- **This phase's verification names a genuinely `(manual)` step** → `${CLAUDE_SKILL_DIR}/references/manual-verification.md`, which carries "Observe the `(manual)` steps" and "Resolve a paused phase" in full. A `(manual)` tag on machine-readable output is an `(auto)` step (see the sanity check under "Execute the phase") and earns no load.

A phase that renders nothing and names no `(manual)` step loads neither — the common case. A reference already read this run is not read again.
...
**Check it, don't sense it.** Take the hash from the newest entry's `**Plan committed at:**` field (in preflight's `entry` slice) and run `git diff <that-hash> -- <plan-path>`.

**No such field?** Fall back to `git log -1 --format=%h --grep='^plan(' -- <plan-path>` — build's own `plan(<slug>): …` commits, never the `plan: <slug>` authoring commit. Empty means nothing has logged yet: no continuity check is owed.
...
- **The plan changed, but not in the phase you're about to run** → an expected mid-flight amendment. Note it in one line ("Phase 4's tasks were rewritten since Phase 2's log entry — not this phase") and execute.
- **The phase you're about to run changed** → show that phase's diff and ask whether to execute the current text — the only question this step may produce.
...
3. **Implement** the change. Stay within CONVENTIONS.md and the plan's stated approach. If the task renders anything a person looks at, apply `references/ui.md` (loaded under "Conditional references") before you write it.
...
If a step is tagged `(manual)` but is actually machine-executable (an HTTP request, a database query, a log check, a CLI output), **reclassify it as `(auto)` and run it yourself.** `(manual)` is reserved for a rendered screen a human or browser tool must look at; backend behavior, API responses and data state never are.
...
`esq plan append-log` requires this block when a phase naming a runnable `(auto)` command completes or pauses only for `manualOutstanding`; a phase unable to supply it cannot complete.

**Verification economy.** Verification exists to catch a break, not to demonstrate diligence:
...
- **A whole-repo suite runs on blast radius, not on ritual.** Decide from what *this* phase touched. A file imported outside its own package — a shared helper, a core module, a type its callers read — earns the whole suite **here, immediately**: the callers it may have broken are exactly the ones this phase's targeted tests don't name. A phase confined to its own package runs that package's suite, and the log entry names the suite it skipped and why. **The plan's last phase runs the whole thing regardless**.
...
- **Start the slow check before the writing, not after it.** Launch the slow run with `run_in_background: true` the moment the last task is committed, draft *What got built*, *Surprises*, *Backlog candidates* and *For Phase N+1* while it runs, then read its result, fill *Verification* from it, and only then commit. **Read the result before anything is committed**: a suite that reds is a failed phase however finished the prose looks.
...
<!-- shared:collect-once:end -->
  - **Every process this phase started is confirmed gone before the execution-log entry is appended.** Process-group ownership, not a single PID.
...
- **A red verification is never logged `completed`, however late or badly the wait was collected**; it gets a failure report.
...
- **Report the bill.** If verification took more than a couple of minutes, put the number in the log entry.
...
**Run "Observe the `(manual)` steps" in `${CLAUDE_SKILL_DIR}/references/manual-verification.md` — the full procedure, probe included** (load it now if "Conditional references" did not). Only the probe decides that you have no instrument, and no step is PASS before it.
...
**Out of scope means outside the shipping *unit* — every plan recording the same `**Branch:**` — not outside this phase.** A backlog row is for work that belongs somewhere else, never a place to park a defect in code this unit wrote (`D-a-defect-against-the-unit-is-not-a-row`). `esq plan append-log` blocks a `completed` entry on any undisposed 🐛/⚠️ whose `Source` names a `build:` or `fix:` step of this unit, wherever the failure lives. So a defect in this unit's code is never a row; it is one of:
...
- **Substantive** — it needs a decision, a design, or work the plan did not buy. **The phase pauses.**
  Append the paused entry with `blockedBy` naming the rows, and stop.

A row the unit's own commit then fixes and verifies, you close:
...
2. **File this phase's backlog candidates first.** Run the "Capture backlog candidates" step now, ending in its own `backlog: capture candidates from phase N (<slug>)` commit. `"None."` files nothing and commits nothing — the common case.
3. **Recompute the unit's open defects** — `esq branch check <plan-path>`, once, here. Preflight's verdict predates this phase's commits and step 2's rows.
4. **`unit.open` non-empty → the paused format, `blockedBy` naming those rows, and no `completed` entry is attempted.** The CLI would refuse one. A red `(auto)` step is still a failure report and logs nothing.
...
**Load the bookkeeping procedures only where this phase owes work in them.** Steps 2 and 6 live in `${CLAUDE_SKILL_DIR}/references/decisions-and-backlog.md`; three branches earn that one load:

- **Candidates at step 2** — `backlogCandidates` is anything but `"None."`. Load it **before any row is filed**.
- **A registry decision at step 6** — a choice in *Surprises / decisions* that later work could break or re-litigate without knowing (a contract, data shape, repeated pattern, trust boundary, product rule, dependency). A local implementation call does not qualify; most phases have none.
- **The plan's last phase, and it completed** — the rows `/esq:plan` stamped ` · Planned by <slug>` are inspected and closed here, and that work is owed even with no candidates and no decisions. Load it and look; never report that nothing matched without having looked.

Otherwise load none of it; steps 1–5 still run exactly as written. A branch you are unsure of loads. A phase that stops at a failure reports from "On failure" and files nothing; one that recovers and reaches step 2 is judged by the three branches like any other.

`esq plan append-log` places the entry at the end of the execution-log span, above any appendix. Use the **completed** format when the phase is fully verified, or the **paused** format when `(manual)` steps remain or step 4 fired.
...
**Verified:** <written by the payload's `verified` field — see "Record proof" above; never by hand>
...
**Verified:** <as the completed format; optional on a pause naming `blockedBy`>
...
- **Distinct causes only — a second edit against a cause this phase already spent one on is exhaustion, not a repair.** Name the cause as you spend on it; a re-labelled repeat of a repaired cause is the same cause and stops the phase. A run reaching `repairs 3/3` reports the count: the plan is the suspect.
...
The report, option set, ask, apply and recovery routes are one procedure in `${CLAUDE_SKILL_DIR}/references/failure-and-recovery.md`: you MUST load it the moment any failure condition above holds, before you write a word of the report, on every route — never reconstruct it from this list or memory. Do not load it otherwise.
...
- Reconciliation costs no phase budget, at most once per invocation (see "Reconcile before executing"); never re-implement a task whose work the tree already carries.
...
- **no subagents** — this command is the executor and applies a chosen option itself.
- Honest reporting only. If something didn't work, say so. Never claim verification you didn't run.
- NEVER self-certify a `(manual)` verification step. Without an actual observation (a tool's output or the user's confirmation), a phase with manual steps is paused, not completed, and flipping `⏸` to `completed` on your own say-so is forbidden.
