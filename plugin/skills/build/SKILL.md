...
1. Execute each `(auto)` step exactly as written — with two exceptions, and only two. **Exception 1 — a whole-repo suite this phase's blast radius does not earn** (see Verification economy below): the step is narrowed to the suite that covers what the phase touched. Plans written before this exception existed name the wide suite in every phase; this is what stops it costing eight minutes four times over. **Exception 2 — a command that cannot run as written**, where the step's own sentence names the artifact and the property being checked: run the check that prose describes, at the same or greater strictness — never a narrower check, and never a different property. A command cannot run as written when its script or executable is missing or is not executable, when the argument shape it is handed is one that script's own documented usage refuses, or when it names a different artifact than the step's prose does. Nothing else in a step is ever rewritten, narrowed or skipped, and the count is the guard: a third exception has to argue with a number rather than slip in beside a list. Neither exception is a repair: it changes which command runs and spends no repair budget.
2. **Record either substitution in the log entry, in the same place and the same shape** — the command you ran, the command the plan named, and why the plan's command could not do the job: for exception 1, why the wider suite could not have caught anything this phase could break; for exception 2, why the named command could not run as written, and the sentence of the step that supplied the artifact and the property. Both ride the existing `verification` field of `esq plan append-log` — no new field, and no second file touched mid-phase. Exception 2 additionally files the unrunnable step as a `backlogCandidates` entry against the plan that shipped it: a step that cannot run is a defect that breaks the next run of `/esq:build` and `/esq:converge`'s landing gate, for whoever runs them.
3. For each: report ✅ PASS with evidence, ❌ FAIL with what you observed, or ⚠️ CAN'T RUN with why. **A command that cannot run as written and does not qualify for exception 2 is ⚠️ CAN'T RUN, and the phase fails** — where the step is only a command, or its prose names no artifact and property, or two readings of its intent would run different checks, the intent cannot be recovered: say that the command could not run as written, and never substitute a check the step did not describe.
...
- **Exceptions are not repairs.** Exceptions 1 and 2 change which command runs, are recorded as they say, and spend nothing. A `grep` step that also matches legitimate mentions is exception 2 when its step's prose names the artifact; otherwise it is ⚠️ CAN'T RUN, and the stop is a diagnosis naming the plan line to correct — never a 🔴.
